...
internal static class WorkoutAiSchemas
{
    public const string Instructions =
        ...
        "Read target RPE when it appears. A column headed APE or LSRPE means RPE. When a table splits Early Set RPE and Last Set RPE into two columns, give the earlier working sets the early value and the last working set the last value. When a legacy header combines RPE/%1RM, a percentage cell is loadText, not RPE; preserve it exactly (including ranges) and leave targetRpe null unless a separate RPE/LSRPE value is stated. A plain RPE value applies to the listed working sets; a separate LSRPE value applies to the final working set. Parse RIR into rir and convert RIR to targetRpe = 10 - RIR for the numeric RPE column. " +
