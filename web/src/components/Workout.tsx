...
export function Workout({ session, preferences, exercises, queue, onSaved, onClose, onFinish, onDiscard }: {
  ...
  async function finish() {
    ...
    setBusy(true);
    try { queue.clear(); const saved = await api.finishWorkout(draft.id, revision.current, retainSwaps); restTimer.skip(); await onFinish(saved); }
  ...
  async function discard() {
    setBusy(true);
    try { queue.clear(); await api.discardWorkout(draft.id); await onDiscard(); }
  ...
  return <Modal title={draft.name} onClose={onClose} wide>
    <div className="workout-summary"><span><Clock3 size={16} />{Math.floor(elapsed / 60)}:{String(elapsed % 60).padStart(2, '0')}</span>
      ...
      {draft.nutritionContext?.cached && <span title="Nutrition was unavailable when this workout started">Nutrition context cached</span>}</div>
    <div className="modal-body workout-body">
      <details className="workout-help"><summary>How to log a set <ChevronDown size={14} /></summary><p>Enter the load, reps, and effort for each working set. Warm-ups stay separate from working volume.</p></details>
...
function ExerciseBlock({ exercise, index, unit, draft, exercises, change, editSet, toggle, onSwap }: {
  ...
  return <section className={`logging-exercise ${exercise.isReplacement ? 'swap-continuation' : ''}`}><div className="section-heading"><div><h3>{exercise.isReplacement && exercise.originalName ? `Continuation · ${exercise.name}` : exercise.name}</h3>{exercise.isReplacement && <span className="tiny-label">Swapped · {exercise.originalName}</span>}{!exercise.exerciseId && <span className="tiny-label">Not in library</span>}</div>
    ...
    {swapOpen && <Modal title={`Swap ${exercise.name}`} onClose={() => setSwapOpen(false)}><div className="modal-body"><p className="source">Completed sets stay with {exercise.isReplacement && exercise.originalName ? exercise.originalName : exercise.name}; remaining sets continue under the replacement.</p>
      ...
      <ExerciseLibrary exercises={exercises} action="swap" onSelect={id => { const chosen = exercises.find(item => item.id === id); if (chosen) void onSwap(exercise.id, chosen.id, chosen.name).then(() => setSwapOpen(false)); }} /></div></Modal>}
    {exercise.progression && <div className="progression-note"><TrendingUp size={14} aria-hidden="true" />
    ...
    <div className="set-table"><div className="set-table-head"><span>Set</span><span>Target</span><span>{unit.toUpperCase()}</span><span>Reps</span><span>RPE</span><span>Log</span><span /></div>
      {exercise.sets.map((set, si) => { const plan = prescription[si] ?? prescription.at(-1); const shown = toDisplay(set.weightKg, unit); const warmup = set.warmup || plan?.warmup; const loadModel = exercise.loadModel ?? 'external'; const loadEditable = loadModel === 'external' || loadModel === 'full_bodyweight' && (set.resistanceMode ?? 'bodyweight') !== 'bodyweight';
        ...
        return <div className={`set-row ${set.suggestion ? 'has-suggestion' : ''} ${set.done ? 'done' : ''} ${warmup ? 'warmup-row' : ''} ${loadEditable ? '' : 'load-locked'}`} key={set.id}><span className="set-number">{warmup ? `W${warmupNumber}` : workingNumber}</span>
