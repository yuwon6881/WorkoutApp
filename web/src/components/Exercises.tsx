...
function CustomExerciseModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  return <Modal title="Create custom exercise" onClose={onClose}>
    <form className="modal-body" noValidate onSubmit={submit}>
      <Field label="Name" value={name} onChange={e => setName(e.target.value)} maxLength={160} autoFocus />
      <div className="form-grid-two"><Field label="Muscle" value={muscle} onChange={e => setMuscle(e.target.value)} maxLength={80} /><Field label="Equipment" value={equipment} onChange={e => setEquipment(e.target.value)} maxLength={80} /></div>
      <TextAreaField label="Instructions (optional)" value={cue} onChange={e => setCue(e.target.value)} maxLength={1000} />
      <div className="form-grid-two"><SelectField label="Load model" value={loadModel} onChange={e => setLoadModel(e.target.value)}><option value="external">External load</option><option value="full_bodyweight">Full bodyweight</option><option value="bodyweight_context_only">Bodyweight context only</option><option value="reps_only">Reps only</option></SelectField><Field label="Load increment (kg)" type="number" min="0" max="50" step="0.5" value={loadStepKg} onChange={e => setLoadStepKg(e.target.value)} /></div>
    ...
    <Modal title={exercise.name} onClose={onClose} wide>
      <div className="modal-body exercise-detail-modal">
        ...
        {insight && <>
          ...
          <div className="section-heading"><h3>Progress</h3><div className="detail-selectors"><SelectField label="Metric" aria-label="Progress metric" value={metric} onChange={e => setMetric(e.target.value as ChartMetric)}><option value="estimated1rm">Estimated 1RM</option><option value="load">Heaviest load</option><option value="volume">Session volume</option><option value="reps">Reps</option></SelectField><SelectField label="Range" aria-label="Progress range" value={range} onChange={e => setRange(e.target.value)}><option value="1m">Last month</option><option value="3m">Last 3 months</option><option value="6m">Last 6 months</option><option value="all">All time</option></SelectField></div></div>
