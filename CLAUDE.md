...
- Global and account-owned exercise libraries with progress insights/history clearing, manual templates/programs with targeted muscle tags, partner-selection superset pairing, and responsive set prescription editing, standalone and program swap restore to default baselines with catalog-swapping substitutions, multi-phase scheduling, and AI PDF extraction with background progress, technique-aware catalog matching, superset detection and pairing, explicit rep bounds, selectable 6–10 RPE, averaged rest ranges, standardized responsive set editing, swipe-to-delete on compact layouts, and uplifted searchable editable review with add/delete week management and default draft/exercise restore that blocks program creation until mappings and issues are resolved.
- Active workout drafts with carousel exercise navigation, focused set logging, progression badges, and quick action pills; weight/rep/RPE logging, exercise substitutions restricted to before set completion with default restoration, completed-session history, and a deadline-based rest timer configured per program/set prescription that starts after a completed set with a quiet two-note alert.
